...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    if (!renderer.gl || !renderer.gl.createVertexArray) return;
    ...
    const loop = t => {
      if(t-lastFrame<1000/30){raf=requestAnimationFrame(loop);return;}
      lastFrame=t;
    ...
    const tryStart = () => {
      if (!reduceMotion.matches && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
  ...
  useEffect(() => {
    ...
    u.uSteps.value = Math.min(detailToSteps(detail),40);
